 ...
 */
function distanceSquared2D(x1: number, z1: number, x2: number, z2: number): number {
  const dx = x2 - x1;
  const dz = z2 - z1;
  return dx * dx + dz * dz;
}
...
function pointToPolylineDistance(
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const { distSq, t } = pointToSegmentDistanceSquared(px, pz, p1[0], p1[2], p2[0], p2[2]);
...
export class FeatureSpatialIndex {
  ...
  findNearest(
    ...
    for (const feature of this.features) {
      ...
      const { distance, closestPoint } = pointToPolylineDistance(x, y, z, feature.points);
